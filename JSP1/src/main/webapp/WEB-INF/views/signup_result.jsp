...
<body>
  ...
  <ul>
    ...
    <li>PW 확인 : <%= request.getParameter("inputPwCheck") %></li>
    <li>이름 : <%= request.getParameter("inputName") %></li>
    <li>나이 : <%= request.getParameter("inputAge") %></li>
  </ul>

  <%-- JSP 주석모양 -> JAVA주석으로 바꿔줌
    -> 랜더링 된 HTML 에서 보여지지 않는 주석

    언제쓸까?
    JSP에 작성된 Java 코드를 주석처리할때 사용
   --%>

  <%-- 
  
  [JSP에서 java 코드 작성시 사용하는 태그]

  <% %> : scriptlet(스크립틀릿)
    - JSP에서 자바 코드를 작성할 수 있게 하는 태그
    - 변수선언, IF, FOR 등 모두 작성가능
    - JSP -> Servlet 변환시
      작성된 형태 그대로 Java Servlet 중간에 작성됨
