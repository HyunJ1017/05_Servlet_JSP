...
<body>
  ...
  <h1>forwerd와 redirect</h1>

  <h3>forward : 요청 위임</h3>

  <pre>
    - 클라이언트의 요청을 받은 Servler/ JSP가
    . 직접 응답하는것이 아닌 다른 Servlet/JSP에
    . HttpServletRequest, HttpSelvetResponese
    . 객체를 넘겨서(위임)대신 응답하게 하는 것

    - 요청 위임시
    RequestDispatcher(요청 발송자)를 이용

    - 요청 위임할 JSP "파일의 경로"를 작성하면
  ...
  <pre>
    - client의 요청을 받은 servlet/JSP 에서
    . 직접 응답하지 않고 다른 Servlet/JSP를 다시 요청하는 것
    . (요청받은 Servlet이 응답해줄 JSP가 없음)

    - 보통 Servlet 요청 처리 후 특정 JSP로 요청을 위임해
    . 결과화면을 응답해 주는것이 아닌
    . 다른 Servlet을 다시 요청하여 다른Servlet의 결과 화면을 응답해줌

    - redirect는 다시 요청하는 것
    . -> 기존 client의 request, response 객체가 사라지고
    .    새로운 request, response 객체가 생성된다.
    .    -> request scope에 세팅된 값을이 모두 사라짐
